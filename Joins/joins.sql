...
FROM cliente cli INNER JOIN venda ven OR cli.CODCLI =
ven.CODCLI;
...
FROM cliente
INNER JOIN venda ON cliente.CODCLI = venda.CODCLI
WHERE EXTRACT(MONTH FROM venda.VENCTO) = 11
AND EXTRACT(YEAR FROM venda.VENCTO) = 2004
...
SELECT cliente.NOME, COUNT(venda.DUPLIC) AS quantidade_de_titulos, SUM(venda.VALOR) AS total_devido
FROM cliente
INNER JOIN venda ON cliente.CODCLI = venda.CODCLI
GROUP BY cliente.NOME;
...
SELECT cliente.NOME, COUNT(venda.DUPLIC) AS quantidade_de_titulos, SUM(venda.VALOR) AS total_devido_apelido
FROM cliente
INNER JOIN venda ON cliente.CODCLI = venda.CODCLI
GROUP BY cliente.NOME;
...
SELECT cliente.NOME, venda.DUPLIC, venda.VALOR, (venda.VALOR * 0.1) AS valor_juros, (venda.VALOR + (venda.VALOR * 0.1)) AS total_a_cobrar
FROM cliente
INNER JOIN venda ON cliente.CODCLI = venda.CODCLI
WHERE venda.VENCTO < '1999-12-31'
ORDER BY cliente.NOME;

SELECT cliente.NOME as CLIENTE, venda.VALOR, ROUND(venda.VALOR * 0.10, 2) AS JUROS , ROUND(venda.VALOR * 1.10, 2) AS TOTAL
FROM cliente INNER JOIN venda 
ON cliente.CODCLI = venda.CODCLI
WHERE venda.VENCTO <= '999-12-31'
ORDER BY CLIENTE;
